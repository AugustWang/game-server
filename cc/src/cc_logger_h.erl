...
init(File) ->
    ...
    case file:open(File, [append, {encoding, utf8}]) of
        {ok,Fd} ->
            {ok, {Fd, File, []}};
        Error ->
            io:format("\ncc_logger file:open/2 error:~p", [Error]),
            Error
    end.
...
handle_event({log, {login, Rid, Ip}}, State) ->
    catch db:execute("INSERT INTO `log_login`(`role_id`, `event`, `ip`, `login_time`) VALUES (~s, 0, ~s, ~s)", [Rid, Ip, util:unixtime()]),
    {ok, State};
...
handle_event({log, {reg, AccountId, Ip}}, State) -> 
    catch db:execute("INSERT INTO `log_reg`(`account_id`, `ctime`, `ip`) VALUES (~s, ~s, ~s)", [AccountId, util:unixtime(), Ip]),
    {ok, State};
...
handle_event({log, {gold, Rid, Type, Num, Rest}}, State) ->
    case Num of
        0 -> ok;
        _ ->
            catch db:execute("INSERT INTO `log_gold` (`rid`, `type`, `num`, `rest`) VALUES (~s, ~s, ~s, ~s);", [Rid, Type, Num, Rest])
    end,
    {ok, State};
